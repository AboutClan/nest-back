...
import { IParticipation, IResult } from 'src/routes/vote2/vote2.entity';
  ...
  static async findClosestPlace(
    ...
    formedClusters.forEach((cluster, i) => {
      ...
      places.forEach((place, j) => {
        const placeLat = (place as any).location.latitude;
        const placeLon = (place as any).location.longitude;
