...
export default class AdminVoteService {
  ...
  async confirm(dateStr: string) {
    const date = DateUtils.strToDate(dateStr).toDate();
  ...
  async voteStatusReset(date: any) {
    try {
      const vote = await findOneVote(DateUtils.strToDate(date).toDate());
    ...
    try {
        ...
        .aggregate([
          {
            $match: {
              date: {
                $gte: DateUtils.strToDate(startDay).toDate(),
                $lte: DateUtils.strToDate(endDay).toDate(),
              },
            },
          },
          {
      ...
      arriveCheckCnt.forEach((info: any) => {
        ...
        if (info.name[0] && info.location[0] === location) {
          if (info.arrived) {
            if (info.date >= DateUtils.strToDate(startDay).toDate()) {
              if (info.status !== 'free') {
